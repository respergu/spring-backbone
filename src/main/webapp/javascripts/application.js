...
App.SearchResultsView = Backbone.View.extend({
	...
	renderItem : function(model) {
		$(this.el).show();
		var view = new App.SearchItemResultView({model : model});
		this.$('ul').append(view.el);
	}
});

App.SearchItemResultView = Backbone.View.extend({
	tagName : 'li',
	events : {
		'click a': 'addFavorite'
	},
...
App.SearchRouter = Backbone.Router.extend({
	...
	initialize : function() {
		new App.SearchResultView({router : this});
...
App.SearchResultView = Backbone.View.extend({
	...
	handleEnter : function(e) {
		if(e.keyCode == 13) {
			this.options.router.navigate('search/' + $(this.el).val(), true);
		}
	}
});